...
function renderDatalistFromTeams(items) {
  ...
  // value = solo nome squadra (per evitare "(Italy)" nel search)
  // label = nome + paese (utile nei suggerimenti)
  dl.innerHTML = items
    .map((t) => {
      const value = t.name;
...
async function showTeam() {
  ...
  if (!r.ok) {
    setMatch(
      `<p class="bad"><em>Errore API (teams): HTTP ${r.status}</em></p>`,
    );
    return;
  }
  if (r.errors) {
  ...
  // 1) Se esiste match esatto (case-insensitive) -> seleziona subito
  const exact = results.find(
    (x) => String(x?.team?.name || "").toLowerCase() === teamName.toLowerCase(),
  );

  // 2) Se c'è un solo risultato -> seleziona subito
  if (exact) {
  ...
  // 3) Altrimenti mostro UNA lista per scegliere (senza ID)
...
async function loadFixtureDetails() {
  ...
  const r = await apiGet(
    `/fixtures?id=${selectedFixture.id}&timezone=Europe/Rome`,
  );
  if (!r.ok) {
    setReferee(
...
/* =========================
   STORICO ARBITRO (uguale logica, ripulito output)
   ========================= */
async function loadRefereeHistory() {
  ...
  function bucketForFixture(f) {
    ...
    if (leagueCountry.toLowerCase() === "italy")
      return "Altre competizioni in Italia";
    if (
      leagueCountry.toLowerCase() === "world" ||
      leagueCountry.toLowerCase() === "europe"
    )
  ...
  try {
    ...
    const baseDate = selectedFixture.date
    ...
    for (const f of lastN) {
      ...
      const cards = await countCardsFromEventsDetailed(
        fixtureId,
        homeId,
        awayId,
      );
    ...
    const optList = document.getElementById("optShowList");
    const optTeam = document.getElementById("optShowTeamDetail");
...
function getLimitForTeams() {
  // usiamo lo stesso select dell'arbitro (refHistoryCount)
  const limitEl = document.getElementById("refHistoryCount");
  let limit = parseInt(limitEl?.value || "10", 10);
  if (Number.isNaN(limit) || limit < 1) limit = 10;

  // performance: ogni partita richiede una chiamata events
  // (es. 30 partite = 30 chiamate per squadra = 60 chiamate)
  // quindi cap per evitare rallentamenti/blocchi API
  const HARD_CAP = 20;
...
function calcGoalsForAgainst(teamId, fixture) {
  ...
  // se non ci sono goal validi, fallback 0
...
async function buildTeamForm(team, limit) {
  ...
  const perFixture = []; // per dettaglio
...
async function loadTeamsForm() {
  ...
  const optList = document.getElementById("optShowTeamList");
  const optCards = document.getElementById("optShowTeamCardsDetail");
...
function renderTeamMatchesList(form) {
  ...
  const items = form.fixtures
    .map((x) => {
      return `<li>${safeHTML(x.date)} — ${safeHTML(x.home)} vs ${safeHTML(x.away)} <em>(${safeHTML(x.comp)})</em> — GF <strong>${safeHTML(x.gf)}</strong> / GS <strong>${safeHTML(x.ga)}</strong></li>`;
    })
    .join("");
...
function renderTeamCardsList(form) {
  ...
  const items = form.fixtures
    .map((x) => {
      ...
      return `<li>${safeHTML(x.date)} — ${safeHTML(x.home)} vs ${safeHTML(x.away)} <em>(${safeHTML(x.comp)})</em> — 🟨 ${safeHTML(c.yellow)} / 🟥 ${safeHTML(c.red)} (Tot ${safeHTML(c.total)})</li>`;
    })
    .join("");
...
// Cache eventi fixture (così non richiamiamo /fixtures/events più volte)
const __FIXTURE_EVENTS_CACHE__ = new Map(); // fixtureId -> events array

async function getFixtureEventsCached(fixtureId) {
  if (!fixtureId) return [];
  if (__FIXTURE_EVENTS_CACHE__.has(fixtureId)) {
    return __FIXTURE_EVENTS_CACHE__.get(fixtureId);
  }
  const r = await apiGet(`/fixtures/events?fixture=${fixtureId}`);
...
function extractCornersFromStatsForTeam(statsArr, teamId) {
  // statsArr: [{team:{id}, statistics:[{type, value}, ...]}, ...]
  const teamBlock = (statsArr || []).find(
    (x) => (x?.team?.id ?? null) === teamId,
  );
  const list = teamBlock?.statistics || [];
  const cornerItem = list.find(
    (s) => String(s?.type || "").toLowerCase() === "corner kicks",
  );
  const v = cornerItem?.value;

  // value può essere numero o stringa o null
  const n = typeof v === "number" ? v : parseInt(String(v ?? "0"), 10);
  return Number.isFinite(n) ? n : 0;
}

async function getCornersForFixtureTeams(fixtureId, homeId, awayId) {
  // cache per non richiamare più volte lo stesso fixture
  if (__FIXTURE_STATS_CACHE__.has(fixtureId))
    return __FIXTURE_STATS_CACHE__.get(fixtureId);

  const r = await apiGet(`/fixtures/statistics?fixture=${fixtureId}`);
  // API-Football: response è array con 2 blocchi (una per squadra)
...
async function buildTeamCorners(team, limit) {
  ...
  for (const f of fixtures) {
    ...
    if (fixtureId && homeId && awayId) {
      ...
      // l’avversario è “l’altro team” di quella partita
    ...
    if (minAgainst === null || cornersAgainst < minAgainst)
      minAgainst = cornersAgainst;
    if (maxAgainst === null || cornersAgainst > maxAgainst)
      maxAgainst = cornersAgainst;
...
function renderTeamCornersList(form) {
  ...
  const items = form.fixtures
    .map((x) => {
      return `<li>${safeHTML(x.date)} — ${safeHTML(x.home)} vs ${safeHTML(x.away)} <em>(${safeHTML(x.comp)})</em> — Corner <strong>${safeHTML(x.corners)}</strong></li>`;
    })
    .join("");
...
function renderTeamCornersPerMatch(form) {
  ...
  const items = form.fixtures
    .map((x) => {
      return `<li>${safeHTML(x.date)} — ${safeHTML(x.home)} vs ${safeHTML(x.away)} <em>(${safeHTML(x.comp)})</em> — Corner <strong>${safeHTML(x.cornersFor)}</strong> fatti / <strong>${safeHTML(x.cornersAgainst)}</strong> subiti</li>`;
    })
    .join("");
...
async function loadTeamsCorners() {
  ...
  const limit = getLimitForTeams(); // stesso cap “sicuro” usato per Squadre
  const requested =
    parseInt(document.getElementById("refHistoryCount")?.value || "10", 10) ||
    10;
  ...
  const optList = document.getElementById("optShowCornersList");
  const optPer = document.getElementById("optShowCornersPerMatch");
  ...
  if (divPer) {
    // in questo MVP è uguale alla lista (corner per match = riga per riga)
    // lo teniamo separato perché magari dopo vuoi aggiungere “corner fatti vs corner subiti” ecc.
    divPer.innerHTML = `
  <hr />
...
document
  .getElementById("optShowTeamCardsDetail")

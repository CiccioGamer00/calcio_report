...
### Dynamic attack/defence locked result — 2026-09-18

The expanded development grid on Serie A 2024/25 selected learning rate `0.075` and blend `1.00`. The learning rate was no longer at the search boundary. Relative to the locked current model, development full-season metrics improved from log loss `1.0505` to `0.9867`, Brier `0.6275` to `0.5904`, RPS `0.2108` to `0.1954`, calibration error `0.0781` to `0.0476`, and 1X2 accuracy `50.79%` to `52.11%`.

Those parameters were then evaluated once, without recalibration, on the untouched Serie A 2025/26 final season. Full-season results again improved: accuracy `47.37%` to `49.21%`, log loss `1.0770` to `1.0221`, Brier `0.6418` to `0.6121`, RPS `0.2198` to `0.2098`, and calibration error `0.0739` to `0.0134`. Minimum-coverage results showed the same direction. In the first 30 matches, log loss, Brier and calibration improved while accuracy fell by `3.33` points and RPS worsened by only `0.0003`; by the first 50 matches, every reported metric improved, including accuracy by `4.00` points.

This is the first opponent-quality experiment to pass both development and locked final-season evaluation. It is accepted as an integration candidate, not yet as a production deployment. The laboratory branch is behind the current `main`, so production work must start from a fresh branch based on the current `main` and port only the validated model behavior. The public app and deployed Worker remain unchanged.
